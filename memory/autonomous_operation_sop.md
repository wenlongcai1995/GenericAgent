...
## 报告质量Checklist（从R1成功实践验证）
完成报告后逐条核对，非缺一不可但建议尽量满足：
- ✅ **版本/赛季背景说明**（让读者快速定位时间上下文）
- ✅ **核心发现 + 定性原因**（不止"是什么"还要"为什么"）
- ✅ **数据来源表**（作者/标题/播放量/日期，可溯源验证）
- ✅ **策略建议/行动指导**（不止信息，还有"所以呢"）
- ✅ **记忆更新建议**（为未来知识复用铺路）
- ✅ **数据源差异标注**（不同来源观点不一致时说明，避免读者困惑）